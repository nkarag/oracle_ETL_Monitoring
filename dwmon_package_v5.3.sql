CREATE OR REPLACE PACKAGE BODY monitor_dw.DWMON_PACKAGE  AS
/******************************
Version 5.3
    filter out from sqlsnap a specific query
Version 5.2
...
3. include ETL statistics --> PENDING
    1. build new snap tables to record ETL info such as:
        ...
        b. Data rejections (see rejection tables and shadow_dw.ETL_LOG)
...
using (
    ...
    from v$sqlstats  s
    where sql_text not like '%PILOT_PRODUCT_CATEG%'  
) sqlstats
