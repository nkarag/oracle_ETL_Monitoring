...
select  
        sum(case when updated_on < trunc(updated_on) + 4/24 then 1 else null end) lt_04,
        round(sum(case when updated_on < trunc(updated_on) + 4/24 then 1 else null end) / count(*),2) lt_04_pct,
        sum(case when updated_on between trunc(updated_on) + 4/24 and trunc(updated_on) + 6/24 then 1 else null end) btween_04_06,
        round(sum(case when updated_on between trunc(updated_on) + 4/24 and trunc(updated_on) + 6/24 then 1 else null end)/count(*),2) btween_04_06_pct,
        sum(case when updated_on > trunc(updated_on) + 6/24 then 1 else null end) gt_06,
        round(sum(case when updated_on > trunc(updated_on) + 6/24 then 1 else null end)/count(*),2) gt_06_pct,
        count(*) num_of_run        
from (
with q1 as ( 
...
select sysdate, trunc(sysdate) + 4/24
from dual